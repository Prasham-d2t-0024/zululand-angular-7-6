...
export class SearchService implements OnDestroy {
  ...
  private searchlinkpathRecent ='discover/search/objects/recentsubmitted'
  private searchLinkPathSuggestion = 'discover/search/suggestion';
  /**
  ...
  getEndpointSuggestion(searchOptions?: PaginatedSearchOptions): Observable<string> {
    return this.halService.getEndpoint(this.searchLinkPathSuggestion).pipe(
      map((url: string) => {
        if (hasValue(searchOptions)) {
          return (searchOptions as PaginatedSearchOptions).toRestUrl(url);
        } else {
  ...
  searchSuggestion<T extends DSpaceObject>(searchOptions?: PaginatedSearchOptions, useCachedVersionIfAvailable = false, responseMsToLive?: number,  reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<T>[]): Observable<RemoteData<SearchObjects<T>>> {
    const href$ = this.getEndpointSuggestion(searchOptions);
  ...
  getEndpointRecent(searchOptions?: PaginatedSearchOptions): Observable<string> {
    return this.halService.getEndpoint(this.searchLinkPath).pipe(
      map((url: string) => {
        if (hasValue(searchOptions)) {
          return (searchOptions as PaginatedSearchOptions).toRestUrl(url);
  ...
  searchRecent<T extends DSpaceObject>(searchOptions?: PaginatedSearchOptions, responseMsToLive?: number, useCachedVersionIfAvailable = true, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<T>[]): Observable<RemoteData<SearchObjects<T>>> {
    const href$ = this.getEndpointRecent(searchOptions);
    let href1 = "";
    href$.pipe(
      take(1),
      map((href: string) => {
         href1 = href.split("?")[0];
        href1 = href1 + "/recentsubmitted?" + href.split("?")[1];
       
        const args = this.searchDataService.addEmbedParams(href1, [], ...linksToFollow);
        if (isNotEmpty(args)) {
          return new URLCombiner(href1, `?${args.join('&')}`).toString();
        } else {
          return href1;
        }
      })
    ...
    const sqr$ = href$.pipe(
      switchMap((href: string) => this.rdb.buildFromHref<SearchObjects<T>>(href1))
    );
