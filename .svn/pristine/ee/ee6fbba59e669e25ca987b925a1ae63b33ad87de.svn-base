 ...
 */
export interface SearchData1<T extends CacheableObject> {
  /**
   ...
   */
  searchBy1(searchMethod: any, options?: FindListOptions, useCachedVersionIfAvailable?: boolean, reRequestOnStale?: boolean, ...linksToFollow: FollowLinkConfig<T>[]): Observable<RemoteData<PaginatedList<T>>>;
}
 ...
 */
export class SearchDataImpl1<T extends CacheableObject> extends BaseDataService<T> implements SearchData1<T> {
  /**
   * @param linkPath
  ...
  constructor(
    ...
    if (hasNoValue(constructSearchEndpoint)) {
      throw new Error(`SearchDataImpl1 initialized without a constructSearchEndpoint method (linkPath: ${linkPath})`);
    }
  }
  ...
  searchBy(searchMethod: string, options: FindListOptions = {}, useCachedVersionIfAvailable = true, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<T>[]): Observable<RemoteData<PaginatedList<T>>> {
    ...
    const newUrl = href.replace(new RegExp(`/${wordToRemove}`, 'g'), '');
      href = newUrl;
  });
    return this.findListByHref(hrefObs, undefined, useCachedVersionIfAvailable, reRequestOnStale, ...linksToFollow);
  }

  searchBy1(searchMethod: string, options: FindListOptions = {}, useCachedVersionIfAvailable = true, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<T>[]): Observable<RemoteData<PaginatedList<T>>> {
    const hrefObs = this.getSearchByHref(searchMethod, options, ...linksToFollow);
    return this.findListByHref1(hrefObs, undefined, useCachedVersionIfAvailable, reRequestOnStale, ...linksToFollow);    
  }
