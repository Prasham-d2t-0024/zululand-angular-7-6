...
export class VocabularyTreeviewComponent implements OnDestroy, OnInit {
  ...
  constructor(
    ...
    private dataService:ChartService
  ) {
  ...
  search() {
    if (isNotEmpty(this.searchText)) {
      ...
      this.dataService.searchASFAsubject(this.searchText,0).pipe().subscribe((data) => {
        this.listView = true;
        this.flatListData = [];
        if (data) {
          // Extract data from the response
          const childData = data['_embedded'] ? data['_embedded']['entries'] : [];
          this.totalPage = data['page'] ? data['page'].totalPages : 0;
          if(this.totalPage !== 1) {
            this.haveMoreData = true;
          }
          for (const i of childData) {
  ...
  loadMoreList() {
    ...
    if(this.totalPage === this.pageCount - 1) {
      this.haveMoreData = false;
    } else {
      this.haveMoreData = true;
    }

    this.dataService.searchASFAsubject(this.searchText,this.pageCount).pipe().subscribe((data) => {
      this.listView = true;
      if (data) {
        // Extract data from the response
        const childData = data['_embedded'] ? data['_embedded']['entries'] : [];
