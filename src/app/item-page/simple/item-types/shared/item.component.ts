...
import { BehaviorSubject, combineLatest as observableCombineLatest, Observable, Subject, of, Subscription, combineLatest } from 'rxjs';
  ...
  findRatingBreaking():void {
   this.commentDataService.findratingBarkingofItem(this.object.id, null, false).pipe(getAllCompletedRemoteData()).subscribe((rd:any) => {
      if (rd.hasSucceeded) {
        ...
        if(hasValue(rd.payload.raitingmap) && Object.keys(rd.payload.raitingmap).length != 0) {
          Object.keys(rd && rd.payload && rd.payload.raitingmap).forEach((key, index) => {
