...
export class MenuResolver implements Resolve<boolean> {
    ...
    this.browseService.getBrowseDefinitions()
      ...
      .subscribe((browseDefListRD: RemoteData<PaginatedList<BrowseDefinition>>) => {
        if (browseDefListRD.hasSucceeded) {
          browseDefListRD.payload.page.forEach((browseDef: BrowseDefinition, index: number, array: BrowseDefinition[]) => {
            // Check if the current index is the last item in the array
  ...
  createAdminMenu$() {
    ...
    this.createReportControlMenuSection();

    return this.waitForMenu$(MenuID.ADMIN);
  }

    createReportControlMenuSection() {
    const menuList = [];
    ...
    observableCombineLatest([
      ...
      //     link: '/access-control/duplicate-item'
      //   } as LinkMenuItemModel,
      //   shouldPersistOnRouteChange: true
      // });
      // this.menuService.addSection(MenuID.ADMIN, {
      ...
      //     text: 'Download Subscribed Email Ids',
      //     function: () => {
      //       let URL = this.contactDataService.downloadEmail();
