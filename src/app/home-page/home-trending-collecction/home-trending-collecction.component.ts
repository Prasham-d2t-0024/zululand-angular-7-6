...
@Component({
  selector: 'ds-home-trending-collecction',
  templateUrl: './home-trending-collecction.component.html',
  styleUrls: ['./home-trending-collecction.component.scss']
})
export class HomeTrendingCollecctionComponent {
   ...
   downloadExcel() {
    this.chartService.downloadZIP().pipe().subscribe((data: any) => {
      this.authService.getShortlivedToken().pipe(take(1), map((token) =>
        hasValue(token) ? new URLCombiner(data +'/report/downloadTrandingMatricxCommunity?dateType=' + this.i + `&collectionorcommunityid=${this.collectionorCommunityId}`+ '&type='+this.type, `&authentication-token=${token}`).toString() : data+'/report/downloadTrandingMatricxCommunity?dateType=' + this.i + `&collectionorcommunityid=${this.collectionorCommunityId}`)).subscribe((logs: string) => {
          window.open(logs);
        });
    },
      (error) => {
