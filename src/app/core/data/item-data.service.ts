...
import { SearchData1, SearchDataImpl1} from './base/search-data1';
...
export abstract class BaseItemDataService extends IdentifiableDataService<Item> implements CreateData<Item>, PatchData<Item>, DeleteData<Item> {
  ...
  private searchData1: SearchData1<Item>;
  ...
  protected constructor(
    ...
    this.searchData1 = new SearchDataImpl1(this.linkPath, requestService, rdbService, objectCache, halService, this.responseMsToLive);
  ...
  public mapToCollection(itemId: string, collectionHref: string): Observable<RemoteData<NoContent>> {
   ...
   public _getprogressReportByDate(fromdate: string, todate: string, collectionId: string, options: FindListOptions = {}, useCachedVersionIfAvailable = false, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<Item>[]): Observable<RemoteData<PaginatedList<Item>>> {
    options = Object.assign({}, options, {
      searchParams: [new RequestParam('startdate', fromdate), new RequestParam('enddate', todate), new RequestParam('collection', collectionId ? collectionId : '')],
    });
...
getDuplicateItem(searchstring:string,searchOptions?: PaginatedSearchOptions): Observable<RemoteData<Item>>  {
  const href$ = this.getRelationshipEndpoint('items').pipe(
    find((href: string) => hasValue(href)),
  );
  let url
  href$.subscribe((data) => {
    const wordToRemove = 'search';
    const newUrl = data.replace(new RegExp(`/${wordToRemove}`, 'g'), '');
    url = newUrl + "/"+searchstring;
  });
  return this.getData(url);
}
getDuplicateSubItem(searchMethod: string, options?: FindListOptions, useCachedVersionIfAvailable?: boolean, reRequestOnStale?: boolean, ...linksToFollow: FollowLinkConfig<Item>[]):  Observable<RemoteData<PaginatedList<Item>>> {
  // const href$ = this.getRelationshipEndpoint('items').pipe(
  //   find((href: string) => hasValue(href)),
  // );
  // let url
  ...
  //   const wordToRemove = 'search';
  //   const newUrl = data.replace(new RegExp(`/${wordToRemove}`, 'g'), '');
  //   url = newUrl + "/getItemByAuthorAndTitle?" + "auther=" + searchstring.author + "&title=" + searchstring.title;
  // });
  // return this.getData1(url);
  debugger;
  return this.searchData1.searchBy1(searchMethod, options, useCachedVersionIfAvailable, reRequestOnStale, ...linksToFollow);
}

private getData(url: string): Observable<RemoteData<Item>> {
  return this.httpClient.get<RemoteData<Item>>(url);
}
private getData1(url: string): Observable<RemoteData<PaginatedList<Item>>> {
  return this.httpClient.get<RemoteData<PaginatedList<Item>>>(url);
}
...
export class ItemDataService extends BaseItemDataService {
  constructor(
    ...
    super('items', requestService, rdbService, objectCache, halService, notificationsService, comparator, browseService, bundleService,httpClient);
  }
}
