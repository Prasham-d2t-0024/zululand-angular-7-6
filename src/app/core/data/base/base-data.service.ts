...
export class BaseDataService<T extends CacheableObject> implements HALDataService<T> {
   ...
   * @param linksToFollow               List of {@link FollowLinkConfig} that indicate which {@link HALLink}s should be automatically resolved
   */
    findListByHref1(href$: string | Observable<string>, options: FindListOptions = {}, useCachedVersionIfAvailable = true, reRequestOnStale = true, ...linksToFollow: FollowLinkConfig<T>[]): Observable<RemoteData<PaginatedList<T>>> {
      if (typeof href$ === 'string') {
        href$ = observableOf(href$);
      }
      ...
      const requestHref$ = href$.pipe(
        ...
        return  this.buildHrefFromFindOptions(newUrl, options, [], ...linksToFollow)
      }
      ),
      );
