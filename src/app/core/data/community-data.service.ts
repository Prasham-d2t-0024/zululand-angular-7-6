...
export class CommunityDataService extends ComColDataService<Community> {
  ...
  findSelected(options: FindListOptions = {}, ...linksToFollow: FollowLinkConfig<Community>[]): Observable<RemoteData<PaginatedList<Community>>> {
   // debugger;
    return this.getEndpoint().pipe(
      map(href => `${href}/search/getCommunityBydisplayRecentSubmission`),
      switchMap(href => this.findListByHref(href, options, false, false, ...linksToFollow))
    );
  }
