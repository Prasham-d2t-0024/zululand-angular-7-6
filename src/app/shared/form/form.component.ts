...
export class FormComponent implements OnDestroy, OnInit {
  ...
  constructor(private formService: FormService,
              ...
              private patchService: ChartService,
   ...
   */
  isSubjectASFA:boolean;
  ngOnInit() {
    if(this.formId==='3_traditionalpagetwo') {
      this.isSubjectASFA = true;
    }
    if (!this.formGroup) {
  ...
  letItem(value:any) {
    if(value.metadataKey === 'dc.subject.asfa') {
      return true;
    } else {
      return false;
    }
  ...
  eraseItem($event, arrayContext: DynamicFormArrayModel, index: number,arrayContext1: DynamicFormArrayModel, index1: number): void {
    const formArrayControl = this.formGroup.get(this.formBuilderService.getPath(arrayContext)) as UntypedFormArray;
      const event = this.getEvent($event, arrayContext, index, 'remove');
      if (this.formBuilderService.isQualdropGroup(event.model as DynamicFormControlModel)) {
        // In case of qualdrop value remove event must be dispatched before removing the control from array
      ...
      this.insertItem($event, arrayContext1, index);
      this.route.paramMap.subscribe(params => {
        const itemNumber = params.get('id'); // 'id' should match the route parameter name in your routing module
        if (itemNumber) {
          // Call the service method with the extracted item number
          this.patchService.removeASFAFromDynamicForm(itemNumber).pipe().subscribe((data) => {
