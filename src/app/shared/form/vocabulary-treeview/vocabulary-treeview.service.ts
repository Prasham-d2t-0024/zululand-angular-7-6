...
export class VocabularyTreeviewService {
   ...
   * @param onlyFirstTime
   */
  loadMoreBySearch(item: VocabularyEntryDetail, selectedItems: string[], list) {
    debugger;
    if (!this.nodeMap.has(item.otherInformation.id)) {
      return;
    }
  ...
  searchByQuery(query: string, selectedItems: string[]) {
    ...
    this.vocabularyService.getVocabularyEntriesByValue(query, false, this.vocabularyOptions, new PageInfo()).pipe(
      getFirstSucceededRemoteListPayload(),
      tap((result) => console.log('First API Response:', result)), 
      mergeMap((result: VocabularyEntry[]) => (result.length > 0) ? result : observableOf(null)),
      mergeMap((entry: VocabularyEntry) =>
        this.vocabularyService.findEntryDetailById(entry.otherInformation.parent, this.vocabularyName).pipe(
          getFirstSucceededRemoteDataPayload(),
          tap((detail) => console.log('Second API Response:', detail))
        )
      ),
