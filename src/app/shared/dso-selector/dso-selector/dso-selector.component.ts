...
export class DSOSelectorComponent implements OnInit, OnDestroy {
  ...
  showTypesofSearachOption:boolean = false;
  selectedOption: string = 'collection';
  constructor(
  ...
  ngOnInit(): void {
    ...
    if(this.types && this.types[0] === 'COLLECTION') {
      this.showTypesofSearachOption = true;
    }
    ...
    this.subs.push(observableCombineLatest(
      ...
      if(this.selectedOption === 'collection'){
        ...
        return this.search2(query, page).pipe(
          map((rd) => {
            if (rd.hasSucceeded) {
              // If it's the first page and no query is entered, add the current DSO to the start of the list
              // If no query is entered, filter out the current DSO from the results, as it'll be displayed at the start of the list already
   ...
   * @param useCache Whether or not to use the cache
   */
   search2(query: string, page: number, useCache: boolean = true): Observable<RemoteData<PaginatedList<SearchResult<DSpaceObject>>>> {
    // default sort is only used when there is not query
    debugger;
    let efectiveSort = query ? null : this.sort;
    return this.searchService.search(
   ...
   */
  selectSingleResult(): void {debugger;
