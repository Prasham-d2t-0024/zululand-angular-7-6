...
import { BehaviorSubject, from as observableFrom, Observable, of as observableOf, Subscription, of, throwError } from 'rxjs';
import { catchError, debounceTime, distinctUntilChanged, map, mergeMap, reduce, startWith, switchMap, take } from 'rxjs/operators';
...
export class CollectionDropdownComponent implements OnInit, OnDestroy {
  ...
  populateCollectionList(query: string, page: number) {
            ...
            return observableFrom(collectionsRD.payload.page).pipe(
              mergeMap((collection: Collection) => collection.parentCommunity.pipe(
                ...
                map((community: Community) => ({
                    ...
                    collection: { id: collection.id, uuid: collection.id, name: this.dsoNameService.getName(collection),parentCommunity:collection.parentCommunityHierarchy }
                  })
                ))),
    ...
    this.subs.push(
      this.searchListCollection$.subscribe((list: CollectionListEntry[]) => {
        ...
        console.log(this.searchListCollection);
  ...
  getfullcommunityName(data: any): Observable<string> {
    ...
    return this.hierachyservice.findhierachiesBycommunity(data.id).pipe(
