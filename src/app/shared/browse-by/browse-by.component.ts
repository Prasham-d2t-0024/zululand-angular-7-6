...
import { BBM_PAGINATION_ID } from 'src/app/browse-by/browse-by-metadata-page/browse-by-metadata-page.component';
 ...
 */
export class BrowseByComponent extends AbstractListableElementComponent<BrowseEntry>  implements OnInit, OnDestroy {
  ...
  public constructor(
    public dsoNameService: DSONameService, private injector: Injector,
  ...
  ngOnDestroy(): void {
   ...
   getQueryParams(author): Observable<Params> {
    ...
    return this.routeService.getQueryParameterValue(pageParamName).pipe(
      map((currentPage) => {
        return {
          value: author.value,
          authority: !!author.authority ? author.authority : undefined,
